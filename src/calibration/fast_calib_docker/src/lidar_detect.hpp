...
public:
    ...
    void detect_mech_lidar(pcl::PointCloud<Common::Point>::Ptr cloud, pcl::PointCloud<pcl::PointXYZ>::Ptr center_cloud)
    {
        ...
        double neighbor_gap_threshold = 0.10;  // 邻近点距离阈值
        int    min_points_per_ring    = 10;    // 太短的 ring 不处理

        ros::param::get("/lidar_detect/neighbor_gap_threshold", neighbor_gap_threshold);
	ros::param::get("/lidar_detect/min_points_per_ring", min_points_per_ring);

	std::cout << "[LiDARDetect] using min_neighbor_gap: " << neighbor_gap_threshold <<  " || min_points_per_ring: " << min_points_per_ring << std::endl;
